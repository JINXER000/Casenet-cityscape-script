...
matRoot='/opt/data/data/citiscape/leftImg8bit/train/';
...
setList = {'train_raw_list'};
for setID = 1:length(setList)
    setName = setList{setID};
 
    % Create output directories
    if(strcmp(setName, 'train_raw_list'))
        for scale = scaleSet
            % Train
    ...
    for idxFile = 1:countFile
        ...
        %img2mat
        %img2mat = imread([fileName  '.png']);
        %filefull = [fileName '.mat'];
        ...
        for idxScale = 1:length(scaleSetRun)
            ...
            %if(strcmp(setName, 'train_file_list'))
                % Write im
                %%
                % _ITALIC TEXT_ age file
                %imwrite(imgScale, [genDataRoot '/image/train/scale_' num2str(scale) '/' num2str(idxFile) '.png'], 'png')
                % Write label file
                %fidLabel = fopen([genDataRoot '/label/train/scale_' num2str(scale) '/' num2str(idxFile) '.bin'], 'w');
